...
TOKEN = '7907155504:AAE_KlRS9FM7jc5eEf0zaquVWQ-vzqTzmcQ'
bot = telebot.TeleBot(TOKEN, threaded=False)

# 🛡️ List of authorized user IDs (replace with actual IDs)
AUTHORIZED_USERS = [7353797869, 5047224084]
...
def start(message):
 ...
 #       types.KeyboardButton('/Show_user_commands'),
  #      types.KeyboardButton('/Show_all_approved_users')
    ...
    bot.send_message(message.chat.id, "👋**WELCOME TO MONSTER BOT**🗿:", reply_markup=markup)
 #   if user_id not in AUTHORIZED_USERS:
        #bot.send_message(message.chat.id, "🚫 Access Denied! Contact the owner for assistance: @Mk_ddos")
...
def owner(message):
    ...
    bot.send_message(message.chat.id, "📞 Contact the owner: @Mk_ddos")
